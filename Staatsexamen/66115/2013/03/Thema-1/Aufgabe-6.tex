...
Funktion \java{hash()} den entsprechenden Streuwert, entnimmt dann den
...
\item Prüfen Sie in einer Methode \java{boolean exists(int h)} der
Klasse \java{zone}, ob bereits mindestens ein Eintrag für einen
gegebenen Streuwert vorhanden ist. Falls \java{h} größer ist als die
derzeitige Größe der Streutabelle, existiert der Eintrag nicht.

\begin{antwort}
\inputcode[firstline=56,lastline=76]{examen/examen_66115_2013_03/Zone}
\end{antwort}
...
\item Die Methode \java{int getIndex (string url, ArrayList<String>
urlList)} soll den Index einer URL in der Kollisionsliste berechnen. Ist
die URL in der Kollisionsliste nicht vorhanden, soll \java{-1}
zurückgeliefert werden.

\begin{antwort}
\inputcode[firstline=68,lastline=88]{examen/examen_66115_2013_03/Zone}
\end{antwort}
...
\item Ergänzen Sie die Klasse Zone um eine Methode \java{String lookup
(String url)}, die in der Streutabelle die IP-Adresse zur \java{url}
zurückgibt. Wird eine nicht vorhandene Adresse abgerufen, wird eine
Fehlermeldung zurückgegeben.

\begin{antwort}
\inputcode[firstline=90,lastline=104]{examen/examen_66115_2013_03/Zone}
\end{antwort}
...
\begin{antwort}
\inputcode[firstline=3]{examen/examen_66115_2013_03/Zone}
\end{antwort}
\end{document}
