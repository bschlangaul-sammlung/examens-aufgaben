...
\path[li graph kante] (a) edge node {6} (d);
\path[li graph kante] (a) edge node {5} (h);
\path[li graph kante] (b) edge node {3} (c);
\path[li graph kante] (b) edge node {4} (e);
\path[li graph kante] (b) edge node {7} (g);
\path[li graph kante] (c) edge (d);
\path[li graph kante] (d) edge node {9} (e);
\path[li graph kante] (d) edge node {6} (f);
\path[li graph kante] (d) edge node {2} (g);
\path[li graph kante] (d) edge node {0} (h);
\path[li graph kante] (e) edge node {5} (f);
\path[li graph kante] (e) edge node {5} (g);
\path[li graph kante] (g) edge node {8} (h);
\path[li graph kante] (h) edge (h);
\end{tikzpicture}
\end{center}
