...
\noindent
Es kann angenommen werden, dass die Klasse \liJavaCode{Verein} bereits
...
\item Es ist eine Implementierung der Klasse \liJavaCode{Spieler} anzugeben.
...
\begin{liAntwort}
\liJavaDatei[firstline=3]{aufgaben/oomup/pu_4/verein/Spieler}
\end{liAntwort}
...
\item Es ist eine Implementierung der Klasse \liJavaCode{Mannschaft}
...
der Methode \liJavaCode{hinzufuegen} um einen Spieler erweitert werden. Die
Methode \liJavaCode{torschuetzenkoenig} soll den Namen eines Spielers aus dem
...
im Team, dann soll der String \liJavaCode{"Kein Spieler vorhanden"}
zurückgegeben werden.

\begin{liAntwort}
\liJavaDatei[firstline=3]{aufgaben/oomup/pu_4/verein/Mannschaft}
\end{liAntwort}
...
\liJavaCode{main}-Methode\index{main-Methode}, so dass nach Ausführung der
Methode eine deutsche Mannschaft existiert mit zwei Spielern Namens
\liJavaCode{"Hugo Meier"} und \liJavaCode{"Frank Huber"}. Beide Spieler sollen zum
selben Verein \liJavaCode{"FC Staatsexamen"} gehören. \liJavaCode{"Hugo Meier"} soll
nach Aufnahme in die deutsche Mannschaft genau ein Tor geschossen haben,
während \liJavaCode{"Frank Huber"} noch kein Tor erzielt hat. (Wir
abstrahieren hier von der Realität, in der ein 2-er Team noch gar nicht
spielbereit ist.)

\begin{liAntwort}
\liJavaDatei[firstline=3]{aufgaben/oomup/pu_4/verein/Verein}
\end{liAntwort}
\end{enumerate}
