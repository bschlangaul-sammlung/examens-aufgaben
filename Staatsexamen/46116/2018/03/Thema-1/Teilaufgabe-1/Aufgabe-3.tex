...
\begin{document}
\bAufgabenTitel{Fußballweltmeisterschaft}
...
\noindent
Es kann angenommen werden, dass die Klasse \bJavaCode{Verein} bereits
...
\item Es ist eine Implementierung der Klasse \bJavaCode{Spieler} anzugeben.
...
\begin{bAntwort}
\bJavaDatei[firstline=3]{aufgaben/oomup/pu_4/verein/Spieler}
\end{bAntwort}
...
\item Es ist eine Implementierung der Klasse \bJavaCode{Mannschaft}
...
der Methode \bJavaCode{hinzufuegen} um einen Spieler erweitert werden. Die
Methode \bJavaCode{torschuetzenkoenig} soll den Namen eines Spielers aus dem
...
im Team, dann soll der String \bJavaCode{"Kein Spieler vorhanden"}
zurückgegeben werden.

\begin{bAntwort}
\bJavaDatei[firstline=3]{aufgaben/oomup/pu_4/verein/Mannschaft}
\end{bAntwort}
...
\bJavaCode{main}-Methode\index{main-Methode}, so dass nach Ausführung der
Methode eine deutsche Mannschaft existiert mit zwei Spielern Namens
\bJavaCode{"Hugo Meier"} und \bJavaCode{"Frank Huber"}. Beide Spieler sollen zum
selben Verein \bJavaCode{"FC Staatsexamen"} gehören. \bJavaCode{"Hugo Meier"} soll
nach Aufnahme in die deutsche Mannschaft genau ein Tor geschossen haben,
während \bJavaCode{"Frank Huber"} noch kein Tor erzielt hat. (Wir
abstrahieren hier von der Realität, in der ein 2-er Team noch gar nicht
spielbereit ist.)

\begin{bAntwort}
\bJavaDatei[firstline=3]{aufgaben/oomup/pu_4/verein/Verein}
\end{bAntwort}
\end{enumerate}
