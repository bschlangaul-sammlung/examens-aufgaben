...
\liLadePakete{graph}
\begin{document}
\liAufgabenTitel{Maximaler Spannbaum mit Jarnik/Prim}
\section{Aufgabe 8
...
\path[li graph kante] (a) edge node {7} (c);
\path[li graph kante] (a) edge node {9} (e);
\path[li graph kante] (a) edge node {2} (f);
\path[li graph kante] (a) edge node {4} (h);
\path[li graph kante] (b) edge node {3} (f);
\path[li graph kante] (b) edge node {5} (h);
\path[li graph kante] (b) edge node {2} (i);
\path[li graph kante] (c) edge node {4} (d);
\path[li graph kante] (c) edge node {4} (e);
\path[li graph kante] (d) edge node {2} (g);
\path[li graph kante] (d) edge node {-3} (h);
\path[li graph kante] (e) edge node {0} (g);
\path[li graph kante] (f) edge node {8} (g);
\path[li graph kante] (f) edge node {10} (h);
\path[li graph kante] (f) edge node {2} (i);
%\path[li graph kante] (i) edge node {20} (i);
\end{tikzpicture}

Wenden Sie den Algorithmus von Jarnik/Prim auf $G$ ausgehend von Knoten
