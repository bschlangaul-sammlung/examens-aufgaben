...
\InformatikPakete{syntax,rmodell}
\begin{document}
...
\ttfamily
\footnotesize
Durchführung(\liPrimaer{Zeit:DATE, RaumNr:INT}, \liFremd{Nr:INT})\\

Lehrveranstaltung(\liPrimaer{Nr:INT}, Name:VARCHAR(20), SWS:INT,
Semester:INT, Art:VARCHAR(20), \liFremd{PersNr:INT})\\

Dozent(\liPrimaer{PersNr:INT}, Name:VARCHR(20),
\liFremd{FakName:VARCHAR(40)})\\
...
belegt(\liFremd{Nr:INT, MatrNr:INT})\\

besitzt(\liFremd{Adresse:VARCHAR(100), MatrNr:INT})\\

ist\_zugeordnet(Studiengang:VARCHAR(40), \liFremd{MatrNr:INT,
FakName:VARCHAR(40)})\\
\end{antwort}
