...
der für eine positive Zahl \liJavaCode{until} die Summe aller Zahlen
bildet, die kleiner als \liJavaCode{until} und Vielfache von $4$ oder
...
\liKontrollTextzeileKnoten{0} - \liKontrollTextzeileKnoten{1} -
\liKontrollTextzeileKnoten{2} - \liKontrollTextzeileKnoten{3} -
\liKontrollTextzeileKnoten{4} - \liKontrollTextzeileKnoten{5} -
\liKontrollTextzeileKnoten{2} - \liKontrollTextzeileKnoten{6}. Der
Übergang von \liKontrollTextzeileKnoten{3} auf
\liKontrollTextzeileKnoten{4} ist hier aber nicht möglich, da beim
ersten Durchlaufen der for-Schleife (\liKontrollTextzeileKnoten{2}) $i$
immer $1$ ist und $1$ weder durch $4$ noch durch $6$ teilbar ist. Somit
kann die Bedingung des inneren ifs (\liKontrollTextzeileKnoten{3}) beim
ersten Durchlauf nie \emph{wahr} sein, womit immer der Übergang
\liKontrollTextzeileKnoten{3} - \liKontrollTextzeileKnoten{5} zu Beginn
genommen werden muss. Alle Pfade, die zu Beginn
\liKontrollTextzeileKnoten{3} - \liKontrollTextzeileKnoten{4} enthalten,
