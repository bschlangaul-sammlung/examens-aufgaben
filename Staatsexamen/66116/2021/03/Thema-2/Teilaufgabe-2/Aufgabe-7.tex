...
\begin{liAntwort}
\begin{tikzpicture}
\node (pi) {$\pi_{\text{Kunde.Name, Kunde.Geburtsdatum}}$};
\node[below=of pi] (sigma) {$\sigma_{\text{Kunde.ID = Rechnung.Kunde} \land \text{Rechnung.Summe < 100}}$} edge (pi);
\node[below=of sigma] (x) {$\times$} edge (sigma);
\node[below left=of x]{Kunde} edge(x);
\node[below right=of x]{Rechnung} edge(x);
\end{tikzpicture}
\end{liAntwort}
...
\begin{tikzpicture}
\node (pi) {$\pi_{\text{Kunde.Name, Kunde.Geburtsdatum}}$};
\node[below=of pi] (theta join) {$\bowtie_{\text{Kunde.ID = Rechnung.Kunde}}$} edge (pi);
\node[below left=of theta join]{Kunde} edge(theta join);
\node[below right=of theta join] (sigma rechnung) {$\sigma_{\text{Summe < 100}}$} edge (theta join);
\node[below=of sigma rechnung]{Rechnung} edge(sigma rechnung);
\end{tikzpicture}
\end{liAntwort}
