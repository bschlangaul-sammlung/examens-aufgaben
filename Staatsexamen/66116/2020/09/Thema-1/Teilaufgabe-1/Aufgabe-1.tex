...
\begin{liAntwort}
Hier fällt die Vorbedingung mit der Invariante zusammen.
Hier muss keine wp-Kalkül berechnet werden, sondern „nur“ die
Zuweisungen nachverfolgt werden um zum Schluss die Nachbedingung
zu erhalten.

Wir haben k mit j ersetzt, damit es besser zum Code passt.
...
\begin{enumerate}
\item $i = -1$ \tmp % 1
\item $i = -1 \land j = 0$ \tmp % 2
\item $i = -1 \land j = 0 \lor j < n$ \tmp % 3
\item $i = -1 \land 0 \leq j < n$ \tmp % 4
\item $i = -1 \land a_j = m \land 0 \leq j < n$ \tmp % 5
\item $i \geq 0 \land a_i = m$ \tmp % 6
\item $i = -1 \land 0 \leq j < n$ \tmp % 7
\item $i = -1 \land \forall \, 0 < j \leq n \colon a_j \neq m$ \tmp % 8
\item $(i = -1 \land \forall \, 0 \leq k < j \colon a_k \neq m) \lor
