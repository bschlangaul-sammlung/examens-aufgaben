...
\liLadePakete{syntax,rmodell,sql}
\begin{document}
\liAufgabenMetadaten{
  Titel = {Aufgabe 3},
...
\begin{liRmodell}
\r{Figur}{\liPrimaer{Id}, Name, Schwertkunst, Lebendig, Titel}

\r{gehört\_zu}{\liPrimaer{Id, Familie},
FK (Id) references Figur(Id),
FK (Familie) references Familie(Id) }

\r{Familie}{\liPrimaer{Id}, Name, Reichtum, Anführer}

\r{Drache}{\liPrimaer{Name}, Lebendig}

\r{besitzt}{\liPrimaer{Id, Name},
FK (Id) references Figur(Id),
FK (Name) references Drache(Name)}

\r{Festung}{\liPrimaer{Name}, Ort, Ruine}

\r{besetzt}{\liPrimaer{Familie, Festung},
FK (Familie) references Familie(Id),
FK (Festung) references Festung(Name)}

\r{lebt}{\liPrimaer{Id, Name},
FK (Id) references Figur(Id),
...
\begin{liSqlErgebnis}
       name       |          titel           | anzahl_titel
------------------+--------------------------+--------------
 Eddard Stark     | Lord von Winterfell      |            2
 Rodd Stark       | Lord von Winterfell      |            2
 Tywin Lennister  | Lord von Casterlystein   |            1
 Cersei Lennister | Lady von Casterlystein   |            1
 Brandon Stark    | König der Andalen        |            1
 Jon Schnee       | König-jenseits-der-Mauer |            1
(6 rows)
\end{liSqlErgebnis}
\end{liAntwort}
