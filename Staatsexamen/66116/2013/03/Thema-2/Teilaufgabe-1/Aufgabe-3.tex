...
\liLadePakete{baum}
\begin{document}
\liAufgabenTitel{Wissensfragen, Ordnung k = 2, Einfügen, Löschen}
\section{Aufgabe 3
\index{B-Baum}
...
\begin{tikzpicture}[
  li bbaum,
  level 1/.append style={level distance=10mm,sibling distance=55mm},
  level 2/.append style={level distance=10mm,sibling distance=18mm}
]
\node{12}[->]
  child{node{6 \nodepart{two} 9}
    child{node {1 \nodepart{two} 2 \nodepart{three} 4 \nodepart{four} 5}}
...
sind.\liFussnoteUrl{http://wwwbayer.in.tum.de/lehre/WS2001/HSEM-bayer/BTreesAusarbeitung.pdf}
\end{liAntwort}
...
\begin{tikzpicture}[
  li bbaum,
  level 1/.append style={level distance=10mm,sibling distance=55mm},
  level 2/.append style={level distance=10mm,sibling distance=16mm}
]
\node{12}[->]
  child{node{\textbf{3} \nodepart{two} 6 \nodepart{three} 9}
    child{node {\textbf{1} \nodepart{two} \textbf{2}}}
