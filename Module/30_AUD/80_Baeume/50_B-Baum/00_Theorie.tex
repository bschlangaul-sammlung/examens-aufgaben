...
\bLadePakete{baum,syntax,spalten}
...
entwickelte B-Baum. Hierbei \bEmph{steht der Name „B“ für balanciert},
...
\begin{enumerate}
\item Jeder Knoten außer der Wurzel enthält zwischen \bEmph{$k$ und $2k$
Elemente} (Schlüsselwerte), $k$ wird als \bEmph{Ordnung} des B-Baums
bezeichnet.
%
\item Jeder \bEmph{Knoten ist entweder ein Blatt (ohne Kinder)} oder hat
\bEmph{mindestens $k + 1$ und höchstens $2k + 1$ Kind-Knoten}.
%
\item Der Wurzelknoten ist \bEmph{entweder ein Blatt oder hat mindestens
2 Nachfolger}.
%
\item Alle Blätter haben die \bEmph{gleiche Tiefe}, \dh alle Wege von
...
Das Einfügen in einen B-Baum erfolgt \bEmph{nur in den Blattknoten}.
Wenn in einem Blattknoten die \bEmph{maximale Anzahl} von Elementen
($2k$) erreicht ist, findet ein \bEmph{Split} statt, \dh die
Elemente werden aufgeteilt und ein neuer Knoten entsteht. Das
\bEmph{mittlere Element} des ursprünglichen Knotens wird dabei \bEmph{in
...
Beginnend mit dem Wurzelknoten werden die Knoten jeweils \bEmph{von
links nach rechts} durchsucht:
%
\bEmph{Stimmt} ein Element mit dem gesuchtem Schlüsselwert
\bEmph{überein}, ist der Satz \bEmph{gefunden}.
%
Ist das \bEmph{Element größer} als der gesuchte Wert, wird die
Suche im \bEmph{links} hängenden Unterbaum \bEmph{fortgesetzt}.
%
Ist das \bEmph{Element kleiner} als der gesuchte Wert, wird der
Vergleich mit dem \bEmph{nächsten Element der Wurzel wiederholt}.
%
Ist auch das letztes Element der Wurzel noch kleiner als der gesuchte
Wert, dann wird die Suche im \bEmph{rechten Unterbaum} des Elements
fortgesetzt.
%
...
\bEmph{genau die minimale Schlüsselanzahl}, ist eine Verschiebung nicht
...
\bEmph{Vaterknoten}, welcher die Wertebereiche der Schlüssel in den beiden zu
...
diesem Fall wird sein \bEmph{symmetrischer Vorgänger} (oder sein
symmetrischer \bEmph{Nachfolger}) gelöscht und \bEmph{an seine Stelle
...
\bEmph{Entscheidung}, in welchen Unterbaum der Abstieg für die Löschung
stattfindet, wird davon abhängig gemacht, \bEmph{welcher genügend
