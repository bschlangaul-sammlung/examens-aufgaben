...
anliegen). Eine solche Tabelle lässt sich durch \liJavaCode{boolean[][]
kanteZulaessig}; in einem zweidimensionalen Feld speichern. Eine
entsprechende Tabelle \liJavaCode{boolean[][] kanteGezeichnet;} erfasst dann die
...
\begin{enumerate}
\item \liJavaCode{void initialsiereFelder()}
\item \liJavaCode{void zeichneKante(int von, int nach)}
\item \liJavaCode{void löscheKante(int von, int nach)}
\item \liJavaCode{void gibLösungAus()}
\item \liJavaCode{void versucheKanteZuZeichnen(int start)}: Die rekursive
...
\liJavaDatei[firstline=3]{aufgaben/aud/muster/backtracking/Nikolaus}
