...
\bLadePakete{graph,syntax,pseudo}
...
Die Tiefensuche (englisch \bEmph{depth-first search}, \bEmph{DFS}) ist
...
Graphen bei dem \bEmph{jeder Knoten mindestens einmal} und \bEmph{jede
Kante genau einmal} besucht wird. Man geht vom jeweiligen Knoten
\bEmph{erst zu einem nicht besuchten Nachbarknoten} und setzt den
Algorithmus dort \bEmph{rekursiv} fort. Bei schon besuchten Knoten wird
abgebrochen. Als Hilfsstruktur wird ein \bEmph{Stack} (Stapelspeicher,
...
\bJavaDatei{graph/algorithmen/TiefenSucheStapel}
...
Der Breitendurchlauf (englisch \bEmph{breadth-first search},
\bEmph{BFS})\footcite{wiki:breitensuche} ist Verfahren zum Durchlaufen
eines Graphen bei dem jeder Knoten genau einmal besucht wird. Man geht
von einem Knoten \bEmph{erst zu allen Nachbarknoten} \bEmph{bevor deren
Nachbarn} besucht werden. Bei schon besuchten Knoten wird abgebrochen.
Als Hilfsstruktur wird eine \bEmph{Queue (Warteschlange)} verwendet.
...
\bJavaDatei{graph/algorithmen/BreitenSucheWarteschlange}
