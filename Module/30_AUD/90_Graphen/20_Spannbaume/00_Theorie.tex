...
\bLadePakete{graph,pseudo,syntax,tabelle}
...
Teilgraph von $G$. $S$ ist ein Spannbaum von $G$, falls $S$ \bEmph{alle
Knoten von $G$ enthält} und $S$ \bEmph{zyklenfrei} ist.
...
und die Summe der \bEmph{Kantengewichte} in $S$ \bEmph{kleiner oder
...
Durch den Algorithmus von Kruskal wird ein \bEmph{minimaler Spannbaum}
...
Spannbäume. Dazu werden die Kanten in der \bEmph{ersten Phase}
\bEmph{aufsteigend nach ihrem Gewicht sortiert}. In der zweiten Phase
wird \bEmph{über die sortierten Kanten iteriert}. Wenn eine
\bEmph{Kante} zwei Knoten verbindet, die \bEmph{noch nicht} durch einen
Pfad vorheriger Kanten \bEmph{verbunden} sind, wird diese Kante zum
minimalen Spannbaum \bEmph{hinzugenommen}.
...
\bJavaDatei{graph/algorithmen/MinimalerSpannbaumKruskal}
...
Vorgehensweise. Er wird auch nach dem \bEmph{Entdecker} Jarník genannt.
\footcite{wiki:prim}

Der \bEmph{Teilgraph} $T$ wird \bEmph{schrittweise vergrößert}, bis
dieser ein Spannbaum ist. Der Algorithmus benötigt einen
\bEmph{konkreten Startpunkt}. Es wird die \bEmph{günstigste} von
einem Teilgraphen $T$ \bEmph{ausgehende Kante ausgewählt} und zu diesem
...
\bJavaDatei{graph/algorithmen/MinimalerSpannbaumPrim}
