...
Die Grundlage für das Black-Box Testverfahren sind \bEmph{Anforderungen
...
des Tests \bEmph{nicht bekannt} sein muss. Die Testobjekte werden daher
\bEmph{unabhängig von ihrer Realisierung getestet}. Aufgrund der
Betrachtung des zu testenden Objekts als Black-Box werden die Testfälle
von \bEmph{Daten getrieben} (\bEmph{Data-Driven}) und beziehen sich auf
...
ist es dabei, eine \bEmph{möglichst hohe Anforderungsüberdeckung} zu
...
sich Techniken, wie \bEmph{Äquivalenzklassenzerlegung} und
\bEmph{Grenzwertanalyse}. Dabei versucht man, die Menge an Eingabe- und
...
Das \bEmph{vollständige Testen} einer Komponente oder eines
...
Eingabewerten} zu identifizieren, die jeweils \bEmph{dieselben
Ergebnisse} liefern. Aus diesen Klassen von Eingabewerte wählt man
jeweils einen \bEmph{Vertreter (Repräsentanten)} aus, der dann für den
...
wieder, dass \bEmph{pro Äquivalenzklassenkombination nur ein
...
Einen besonderen Stellenwert nehmen \bEmph{Bereichsgrenzen von
Äquivalenzklassen} ein, da diese häufige Ursachen für Fehler sind, die
beispielsweise durch \bEmph{Tippfehler} verursacht werden. Statt eines
...
\bEmph{Grenzbereiche zu identifizieren} und (b) \bEmph{Testdaten aus dem
...
Ein Kontrollflussgraph (englisch: \bEmph{control-flow graph} (CFG))
bezeichnet einen \bEmph{gerichteten Graphen} der dazu dient, den
...
Jeder Graph enthält genau \bEmph{einen Startknoten} und \bEmph{einen
Endknoten}. \bEmph{Verschiedenen Aussprungspunkte} (\zB mehrere
return-Anweisungen) werden in einem neuen, \bEmph{zusätzlich
eingeführten Knoten} zusammengeführt.\footcite[Seite
203]{hoffmann:software}

Der \bEmph{kantenmarkierter} Kontrollﬂussgraphen ist ein Graph, indem
die \bEmph{Anweisungen den Knoten} und die
\bEmph{Verzweigungsbedingungen den Kanten} zugeordnet werden. Die
meisten White-Box-Tests basieren auf dieser Art der
Kontrollﬂussmodellierung.\footcite[Seite 203]{hoffmann:software}

\bEmph{Expandierte Kontrollﬂussgraphen} enthalten für jeden Befehl einen
separaten Knoten.\footcite[Seite 204]{hoffmann:software}

\bEmph{Kollabierte Kontrollﬂussgraphen} zeichnen sich dadurch aus, dass
...
\bEmph{Ablaufdiagramm} genannt. Gestrichelte Kanten können
eingezeichnet werden, um einen \bJavaCode{false}-Zweig von einem
\bJavaCode{true}-Zweig besser unterscheiden zu können. Oftmals sind die
...
\begin{itemize}
\item \bJavaCode{if (B) X;}

\item \bJavaCode{if (B) X; else Y;}

\item \bJavaCode{while (B) X;}

\item \bJavaCode{do X; while (B);}
\end{itemize}

\def\beispiel{
\begin{center}
\begin{minipage}{4cm}
\bJavaDatei[firstline=4,lastline=16]{testen/Beispiel}
\end{minipage}
\begin{minipage}{4cm}
\begin{liKontrollflussgraph}[y=-1cm]
\node at (-2,1) (S) {S};
\node at (-1,1.5) (1) {1};
\node at (0,2) (2) {2};
\node at (1,3) (3) {3};
...
\path (S) -- (1);
\path (1) -- (2);
\path[wahr] (2) -- (3) \b{right,pos=0}{y > x};
\path[falsch] (2) -- (5) \b{left}{y <= x};
\path (3) -- (4);
\path (4) -- (5);
\path[wahr] (5) -- (6) \b{right,pos=0}{2 * max < 3 * min};
\path[falsch] (5) -- (E) \b{left}{2 * max >= 3 * min};
\path (6) -- (E);
...
Anweisungsüberdeckungstests, auch $C_0$-Test genannt, testen \bEmph{jede
Anweisung mindestens ein Mal}. Wurde jede Anweisung in einem Programm
mindestens einmal ausgeführt, spricht man von \bEmph{vollständiger
Anweisungsüberdeckung}. Wurde vollständige Anweisungsüberdeckung
erreicht, dann steht fest, dass \bEmph{kein toter Code} (Anweisungen,
...
\noindent
\bJavaCode{max(4, 5);}: \p{S-1-2-3-4-5-6-E}
...
Die Zweigüberdeckung (auch Kantenüberdeckung) fordert, dass jede
\bEmph{Kante} des Kontrollflussgraphen von mindestens einem Testfall
durchlaufen werden muss. Um das Kriterium zu erfüllen, müssen die
Testfälle so gewählt werden, dass jede Verzweigungsbedingung mindestens
\bEmph{einmal wahr} und mindestens \bEmph{einmal falsch} wird. Da
hierdurch alle Knoten ebenfalls mindestens einmal besucht werden müssen,
ist die \bEmph{Anweisungsüberdeckung} in der Zweigüberdeckung
\bEmph{vollständig enthalten}.\footcite[Seite 209]{hoffmann:software}

Der Zweigüberdeckungstest wird auch
\bEmph{Entscheidungsüberdeckungstest} genannt, da die Hilfsvariable
mindestens einmal mit dem Wert \bJavaCode{true} und \bJavaCode{false}
...
White-Box-Prüftechnik, besitzt aufgrund ihrer \bEmph{immensen
Komplexität} aber nur eine äußerst \bEmph{geringe Praxisbedeutung}.
...
wenn für \bEmph{jeden möglichen Pfad}, der den Eingangsknoten des
...
Hilfe sich die \bEmph{kombinatorische Explosion} zumindest teilweise
eindämmen lässt. Von Bedeutung sind in diesem Zusammenhang insbesondere
die \bEmph{Boundary-Interior-Pfadüberdeckung} und die
\bEmph{strukturierte Pfadüberdeckung}.
...
\bEmph{Schleifenkörper nicht betreten}. Für Schleifen fester Lauflänge
...
Die Testfälle durchlaufen das Programm auf Pfaden, die den
\bEmph{Schleifenkörper betreten}, jedoch zu \bEmph{keiner Wiederholung}
führen. Innerhalb des Schleifeninneren müssen \bEmph{alle möglichen
Pfade getestet} werden. Für Schleifen fester Lauflänge ist diese
Testfallgruppe leer.

\item[Innere Pfade (interior test)]

Die Testfälle durchlaufen das Programm auf Pfaden, die den
Schleifenkörper betreten und \bEmph{mindestens eine weitere Iteration}
ausführen. Die Testfälle werden so gewählt, dass \bEmph{innerhalb der
ersten beiden Ausführungen alle möglichen Pfade abgearbeitet}
...
\bPseudoUeberschrift{vollständige Pfadüberdeckung}
...
Datenfluss ab. Hierzu werden die \bEmph{Variablenzugriffe} des
untersuchten Programms analysiert und \bEmph{mit verschiedenen
Datenflussattributen versehen}. Die vergebenen Attribute teilen die
Variablenzugriffe in \bEmph{drei Klassen} ein:
...
In diese Klasse fallen alle Zugriffe, die den aktuellen \bEmph{Wert
...
\bEmph{verwenden, jedoch nicht verändern}. Zwischen einer Zuweisung und
...
In diese Klasse fallen alle Zugriffe, die den \bEmph{Wert einer
Variablen in einer Berechnung verwenden} (\bEmph{computational use},
...
In diese Klasse fallen alle Zugriffe, die den Wert einer Variablen
\bEmph{prädikativ}, d. h. innerhalb eines \bEmph{booleschen Ausdrucks}
verwenden (\bEmph{predicative use}, kurz \texttt{p-use}). Zwischen einer
...
\bJavaDatei[firstline=18,lastline=25]{testen/Beispiel}
...
Mit dem Begriff Unit wird eine \bEmph{atomare Programmeinheit}
bezeichnet, die groß genug ist, um als solche eigenständig getestet zu
werden. Der Unit-Test wird nicht selten auch als \bEmph{Modultest} oder
noch allgemeiner als \bEmph{Komponententest} bezeichnet. Wird die
