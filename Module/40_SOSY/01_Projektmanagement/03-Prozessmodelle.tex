...
ein \bEmph{strikt sequenzieller Projektablauf} möglich ist. Namensgebend
...
\bEmph{vorangegangene Phase vollständig abgeschlossen} und freigegeben
werden muss, \bEmph{bevor die nächste Phase gestartet} werden kann. Als
\emph{Abschluss jeder Phase} sind Verifikations- und
\bEmph{Validierungsschritte} vorgesehen, die quasi als Freigabe der
...
sequenziellen Ablauf eine \bEmph{finale Software-Lösung erst sehr spät
...
gelegentlich noch anzutreffen, wird aber zunehmend \bEmph{von
...
Das V-Modell organisiert \bEmph{ähnlich dem Wasserfallmodell} den
Softwareentwicklungsprozess in \bEmph{Phasen}. \bEmph{Zusätzlich} zu
diesen Entwicklungsphasen definiert das V-Modell auch das Vorgehen zur
\bEmph{Qualitätssicherung (Testen)}, indem den einzelnen
\bEmph{Entwicklungsphasen Testphasen gegenüber gestellt} werden. Auf der
...
bei großen und \bEmph{komplexen Systemen} eingesetzt, von denen rasch
erste verwendbare Teile ausgeliefert werden sollen. Ein Hauptziel ist
dabei, möglichst \bEmph{rasch} mit einer \bEmph{(Teil-)Lösung} auf den
Markt zu kommen und dann das Software-Produkt \bEmph{durch laufende
Ergänzungen auszubauen}.

Die Eckpunkte der Entwicklung sind: \bEmph{Analyse}, \bEmph{Design},
\bEmph{Implementierung}, \bEmph{Integration} und \bEmph{Auslieferung}.
Dabei werden \bEmph{alle Phasen für eine Version} (oder ein Release)
durchlaufen, bis diese Version abgeschlossen ist und ausgeliefert werden
kann. \bEmph{Parallel bzw. zeitlich versetzt} kann bereits mit der
Planung und Umsetzung der \bEmph{Folgeversion} begonnen werden.
...
Das Spiralmodell stellt eine \bEmph{konkrete Ausprägung der
inkrementellen Entwicklung} dar, in dem \bEmph{vier grundlegende
...
\bEmph{Kritikpunkte an traditionellen Vorgehensmodellen} sind unter
...
\bEmph{höheres Maß an Flexibilität und Kundennähe} bei einem
\bEmph{Mindestmaß an Dokumentation} ermöglichen. Agile
Entwicklungsprozesse gehen auf das \bEmph{Agile Manifest}, das von 17
...
\begin{description}
\item[Pregame] In einem \bEmph{Pregame} erfolgt die Festlegung der
...
werden \bEmph{gemeinsam mit dem Kunden} in einem Produkt-Backlog
gesammelt und priorisiert. In diesem \bEmph{Produkt-Backlog} werden auch
geänderte Anforderungen gesammelt.

\item[Sprint] Die eigentliche Entwicklungsarbeit wird in einem
\bEmph{Sprint} durchgeführt. Vor einem Sprint werden die
\bEmph{wichtigsten und machbaren Features} aus dem priorisierten
...
\bEmph{Sprint-Backlog} beinhaltet also einen \bEmph{machbaren Auszug aus
dem Produkt-Backlog}.

\item[Postgame] Die abschließende Phase – das \bEmph{Postgame} – umfasst
die \bEmph{Bereitstellung und Auslieferung neuer Funktionalität} in Form
