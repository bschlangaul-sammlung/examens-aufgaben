...
Software-Architektur ist ein \bEmph{Spezialgebiet des
Software-Engineering}, spielt dabei aber eine wesentliche Rolle. Durch
die Strukturierung werden \bEmph{klare Grenzen in eine Anwendung}
gebracht.

Moderne Software-Systeme sind während der Einsatzzeit meist auch
\bEmph{kontinuierlichen Änderungen} unterworfen. Die
Software-Architektur beschreibt daher nicht nur die statische Struktur
einer Anwendung, sondern es fließen auch \bEmph{nicht funktionale
...
\bEmph{zukünftige Änderungen und Weiterentwicklungen der Software}.
...
Die Schichtenarchitektur zählt zu den \bEmph{beliebtesten
Architektur-Stilen} in der Software-Entwicklung. Jede Schicht
(\bEmph{Layer}) nimmt dabei eine klar definierte Rolle ein und bietet
...
\begin{itemize}
\item Eine Schicht \bEmph{verbirgt} sowohl \emph{darunterliegende
Schichten} als auch die \bEmph{interne Komplexität}. \bEmph{Untere}
Schichten konzentrieren sich meistens auf die \bEmph{Technik}, während
\bEmph{obere} Schichten eher den Fokus auf die
\bEmph{Benutzerschnittstelle} legen.

\item \bEmph{Top-down-Kommunikation}, d. h., Komponenten der höheren
Schicht verwenden Dienste der unteren Schicht und nicht umgekehrt.

\item Komponenten \bEmph{innerhalb} einer Schicht sind von ähnlichem
\bEmph{Abstraktionsgrad} (\zB Komponenten in der Daten-Schicht
konzentrieren sich um Persistenz).

\item Das Design einer Schicht soll eine \bEmph{lose Kopplung} zu
anderen Schichten ermöglichen.

\item Die \bEmph{Kommunikation} zwischen den Schichten erfolgt über klar
\bEmph{definierte Schnittstellen und Protokolle}.
...
\item \bEmph{Einfaches}, effizientes und verständliches
\bEmph{Architekturmuster}.

\item Es besteht eine \bEmph{saubere Trennung} der einzelnen Schichten.
Dadurch kann auch ein verteiltes Arbeiten in Team durchgeführt werden.

\item Schichten sind \bEmph{austauschbar} und können auch auf
...
\item \bEmph{Minimale Abhängigkeit} zwischen den Schichten. Die
Kommunikationswege der Schichten sind durch Schnittstellen klar
definiert.

\item Die \bEmph{Wartung und Erweiterbarkeit} ist durch die klare
...
kann dazu führen, dass \bEmph{aus relativ einfachen Operationen komplexe
Funktionen} werden.

\item Mehr Schichten bedeutet \bEmph{mehr Komplexität und
...
Änderungen in den oberen Schichten und damit einen \bEmph{erhöhten
...
\bEmph{Client/Server-Architektur} bekannt. Der \bEmph{Client} ist dann
in der Regel ein Programm, das sowohl die \bEmph{GUI} als auch die
gesamte \bEmph{Applikationslogik} beinhaltet. Der \bEmph{Server} besteht
meist aus einer \bEmph{relationalen Datenbank}. Für eine
\bEmph{Stammdatenverwaltung} oder ähnliche Systeme war \bEmph{diese
Architektur unter Umständen ausreichend} und auch effizient. Durch die
\bEmph{wachsende Komplexität} in den einzelnen Domänen und die
Anforderungen an verteilte Systeme ist es jedoch meist notwendig und
auch sinnvoll, Teile des Applikationscodes \bEmph{auf mehrere Schichten
...
ein System zu strukturieren. Der \bEmph{Wunsch den Code der
Geschäftslogik vom Client zu lösen}, führt dazu, dass eine neue Schicht
(\bEmph{Logik- oder Business-Schicht}) eingeführt wird, die diese Logik
...
\bEmph{Daten-Schicht}, die eine Menge an Datenquellen zur Verfügung stellt.
...
\bEmph{Logik-Schicht}, welche die Kernfunktionalität des Systems
...
Die Logik-Schicht wird bei \bEmph{komplexen Systemen} oft noch in eine
\bEmph{Datenzugriffs-Schicht} (\bEmph{Data Access Layer}) und eine
\bEmph{Service-Schicht} unterteilt.

Die Datenzugriffs-Schicht besteht aus einer Menge von
\bEmph{Datenzugriffsobjekten}, sogenannten \bEmph{Data Access Objects
...
\bEmph{Präsentationsschicht}. Komponenten in dieser Schicht beinhalten
