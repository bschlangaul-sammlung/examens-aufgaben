...
\bFussnoteUrl{https://www.uni-trier.de/fileadmin/fb4/prof/INF/TIN/Folien/GTI_II/ws_2011/VL4.pdf}

\end{description}

Der Kellerspeicher ist als \bEmph{Stapel (stack)} organisiert und
...
wieder entfernt (\bEmph{LIFO-Prinzip}, \bEmph{Last-In-First-Out}).

Im Gegensatz zu den bisher betrachteten Akzeptoren besitzen
Kellerautomaten zwei \bEmph{verschiedene Alphabete} $\Sigma$ und
$\Gamma$. Das \bEmph{Eingabealphabet $\Sigma$} enthält die Zeichen, aus
denen sich die \bEmph{Eingabewörter} zusammensetzen, und das
\bEmph{Kelleralphabet $\Gamma$} alle Symbole, die in den
\bEmph{Kellerspeicher} geschrieben werden dürfen. Damit beide Alphabete
einfacher unterschieden werden können, halten wir uns an die gängige
Konvention, \bEmph{Eingabezeichen mit Kleinbuchstaben} und
\bEmph{Kellerzeichen mit Großbuchstaben} darzustellen.
...
Die \bEmph{Zustandsübergänge} sind charakterisiert durch das
\bEmph{gelesene Zeichen}, das \bEmph{oberste Kellersymbol} und
\bEmph{die auf den Keller gelegten Zeichen, die das oberste Kellersymbol
ersetzen}.
...
\bEmph{Buchstabe} des Wortes, der \bEmph{gerade bearbeitet} wird.

\item

das \bEmph{aktuelle Kellerzeichen} (\dh das oberste
Zeichen auf dem Keller-Stapel)

\item

\bEmph{Zustand} des Kellers \bEmph{nach Abarbeitung}. Dabei bedeutet...
...
\end{description}
\end{itemize}
\bFussnoteUrl{http://sibiwiki.de/wiki/index.php?title=Kellerautomat}
...
mit nur \bEmph{zwei Zuständen}, wobei die einzige Aufgabe des
Startzustands (\bEmph{\z0}) darin besteht, das \bEmph{Startsymbol $S$
...
\bFussnoteUrl[Seite 29]{https://user.phil.hhu.de/~petersen/Einf_CL_0910/material/EinfCL_14_handout.pdf}
