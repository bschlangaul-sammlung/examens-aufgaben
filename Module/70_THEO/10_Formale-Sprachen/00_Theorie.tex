...
\bLadePakete{mathe,formale-sprachen,tabelle}
\usepackage{tikz}
...
Noam \bEmph{Chomsky} ein Regelwerk, mit dessen Hilfe sich formale
Grammatiken in \bEmph{vier Klassen} einteilen lassen:
...
Insbesondere unterliegt die Struktur der Produktionen \bEmph{keinen
...
\rightarrow r$ entweder die Beziehung \bEmph{$|r| \geq |l|$} erfüllt
oder die Form \bEmph{$S \rightarrow \varepsilon$} aufweist. Ist die Regel
...
Typ-2-Grammatiken sind dadurch charakterisiert, dass die \bEmph{linke
Seite} einer Produktionsregel ausschließlich aus einer \bEmph{einzigen
...
\bEmph{leeren Wort $\varepsilon$} oder einem \bEmph{Terminalsymbol},
\bEmph{gefolgt von einem Nonterminal}, besteht. Formal gesprochen
...
\begin{itemize}
\item \bAusdruck[L_3]{(ab)^n}{n \in \mathbb{N}}
ist regulär (Typ 3 Sprache)

\item \bAusdruck[L_2]{a^n b^n}{n \in \mathbb{N}}
ist kontextfrei (Typ 2 Sprache), aber nicht regulär

\item \bAusdruck[L_1]{a^n b^n c^n}{n \in \mathbb{N}}
ist kontextsensitiv (Typ 1 Sprache), aber nicht kontextfrei

\item \bAusdruck[L_0]{a^{(2^n)}}{n \in \mathbb{N}}
...
Ein \bEmph{Terminalsymbol} (auch Terminalzeichen oder kurz Terminal
genannt) einer formalen Grammatik ist ein Symbol, das einzeln
\bEmph{nicht weiter durch eine Produktionsregel ersetzt werden
...
wie \bEmph{aus Wörtern durch eine Grammatik neue Wörter bzw.
...
Das Produkt \bAusdruck[]{uv}{u \in L_1 \lor v \in L_2} zweier
...
Entscheidungsproblem, zu einem \bEmph{gegebenen Wort} festzustellen, ob
dieses \bEmph{zur Sprache gehört}, oder nicht. Das Wortproblem einer
...
in Form einer formalen Grammatik gegebene \bEmph{formale Sprache $L$
...
\bEmph{äquivalent} sind, also $L(G_1) = L(G_2)$ gilt.
