...
\bLadePakete{mathe,syntaxbaum,automaten,formale-sprachen}
\usepackage{tikz}
...
\begin{itemize}
\item Einer endlichen Menge $V$ von \bEmph{Variablen} (Nonterminale)

\item Dem endlichen \bEmph{Terminalalphabet} $\Sigma$ mit $\Sigma \cap V
= \emptyset$

\item Der endlichen Menge an \bEmph{Produktionen}

\item Und einer \bEmph{Startvariablen} $S$ mit $S \in V$
...
\bigskip
\noindent
\bGrammatik{variablen={Z, A}, alphabet={0, 1}, produktionen={Z -> 1 A | 1,
A -> 1 A | 0 A | 1}}
\end{liKasten}
...
\bProduktionen{A -> EPSILON} oder
\bProduktionen{A -> 1} oder
\bProduktionen{A -> 0 A}
...
Gegebene Grammatik: \bGrammatik{variablen={Z, A}, alphabet={0, 1}, produktionen={Z -> 1 A | 1,
A -> 1 A | 0 A | 1}}
...
\bFussnoteUrl{http://www.informatikseite.de/theorie/node50.php}
...
\bFussnoteUrl{http://u-helmich.de/inf/kursQ21/27/folge-27-2.html}

Eine Sprache $L$ heißt eindeutig, wenn es für $L$ eine eindeutige
Grammatik gibt. Ansonsten heißt $L$ mehrdeutig.
\bFussnoteUrl{http://wwwmayr.in.tum.de/lehre/2008WS/ads-ei/2008-11-24.pdf}
...
Eine Sprache $L$ ist genau dann regulär, wenn sie \bEmph{endlich viele
Äquivalenzklassen} erzeugt.\footcite{wiki:satz-myhill-nerode}

Die \bEmph{Anzahl der Äquivalenzklassen} einer Sprache entspricht der
\bEmph{minimalen Anzahl von Zustanden} eines deterministen-endlichen
...
\bEmph{beiden Fallen} in einen (nicht notwendig gleichen)
\bEmph{Endzustand} oder in beiden Fallen in einen (nicht notwendig
gleichen) \bEmph{Nicht-Endzustand} übergeht.
%
Äquivalente Zustände können \bEmph{zu einem Zustand verschmolzen}
werden.
...
\bFussnoteUrl{https://www.inf.hs-flensburg.de/lang/theor/regulaer-abgeschlossen.htm}
...
Das Produkt \bAusdruck[]{uv}{u \in L_1 \lor v \in L_2} zweier regulärer
