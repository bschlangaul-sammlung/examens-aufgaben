...
\bEmph{Komplexität} bezeichnet die „Kompliziertheit“ von Problemen,
Algorithmen und Daten.
%
Die \bEmph{Komplexitätstheorie} befasst sich dabei mit dem
Ressourcenverbrauch von Algorithmen.
%
Die betrachteten Ressourcen sind fast immer die Anzahl der
benötigten Rechenschritte (\bEmph{Zeitkomplexität}) oder der
Speicherbedarf (\bEmph{Platzkomplexität}).
%
Algorithmen und Probleme werden in der Komplexitätstheorie gemäß ihrer
so bestimmten Komplexität in so genannte \bEmph{Komplexitätsklassen}
...
% 3min
\item[P]
enthält alle Probleme, für die eine \bEmph{deterministische}
Turingmaschine in \bEmph{polynomieller} Zeit eine Lösung berechnen kann.
\footcite[Seite 59]{theo:fs:4}

\item[NP]
% 5min
enthält alle Probleme, für die eine \bEmph{nicht-deterministische} (=
NP) Turingmaschine in \bEmph{polynomieller} Zeit eine Lösung berechnen
kann.
...
\bEmph{Reduktion} benutzt werden kann, um alle Probleme in NP zu
...
Informatik Problemreduktionen benutzt: Ein \bEmph{Problem $A$ heißt
reduzierbar} auf ein anderes Problem $B$, wenn jeder Algorithmus, der
\bEmph{B löst, auch verwendet werden kann, um $A$ zu lösen}, indem man
eine \bEmph{Probleminstanz von $A$ umrechnet in eine Instanz von $B$}
...
\footcite{wiki:polynomialzeitreduktion}

$L_1$ heißt polynomiell reduzierbar auf $L_2$ ($L_1 \leq_p L_2$ ), wenn
es eine total, berechenbare Funktion $f = \Sigma^* \rightarrow \Sigma^*$
gibt, so dass gilt:
%
$w \in L_1 \Leftrightarrow f(w) \in L_2$
%
Es gibt eine deterministische Turingmaschine $M$, die $f$ in
Polynomialzeit berechnet. $L_2$ ist also \emph{„mindestens genauso
schwer“} wie $L_1$.
...
Stephen A. Cook zeigte, dass eine \bEmph{Teilmenge der Klasse NP
existiert}, auf die sich alle \bEmph{Probleme aus NP reduzieren} lassen.
...
benannte Satz von Cook sagt aus, dass das \bEmph{Erfüllbarkeitsproblem
...
können, die also \bEmph{genauso schwer sind wie SAT}.
...
\begin{description}
\item[SATISFIABILITY:] das Erfüllbarkeitsproblem der Aussagenlogik für Formeln in konjunktiver Normalform
\begin{description}
    \item[CLIQUE:] Cliquenproblem
\begin{description}
        \item[SET PACKING:] Mengenpackungsproblem
        \item[VERTEX COVER:] Knotenüberdeckungsproblem
\begin{description}
            \item[SET COVERING:] Mengenüberdeckungsproblem
            \item[FEEDBACK ARC SET:] Feedback Arc Set
            \item[FEEDBACK NODE SET:] Feedback Vertex Set
            \item[DIRECTED HAMILTONIAN CIRCUIT:] siehe Hamiltonkreisproblem
\begin{description}
                \item[UNDIRECTED HAMILTONIAN CIRCUIT:] siehe Hamiltonkreisproblem
\end{description}
\end{description}
\end{description}
    \item[0-1 INTEGER PROGRAMMING:] siehe Integer Linear Programming
    \item[3-SAT:] siehe 3-SAT
\begin{description}
        \item[CHROMATIC NUMBER:] graph coloring problem
\begin{description}
            \item[CLIQUE COVER]
            \item[EXACT COVER:] Problem der exakten Überdeckung
\begin{description}
                \item[3-dimensional MATCHING:] 3-dimensional matching (Stable Marriage mit drei Geschlechtern)
                \item[STEINER TREE:] Steinerbaumproblem
                \item[HITTING SET:] Hitting-Set-Problem
                \item[KNAPSACK:] Rucksackproblem
\begin{description}
                    \item[JOB SEQUENCING:] Job sequencing
                    \item[PARTITION:] Partitionsproblem
                        \item[- MAX-CUT:] Maximaler Schnitt
\end{description}
\end{description}
\end{description}
\end{description}
...
haben, soll eine \bEmph{Teilmenge ausgewählt} werden, deren
Gesamtgewicht eine vorgegebene \bEmph{Gewichtsschranke nicht
überschreitet}. Unter dieser Bedingung soll der \bEmph{Nutzwert} der
ausgewählten Objekte \bEmph{maximiert} werden.
...
die Knoten zueinander \bEmph{nicht adjazent} (= keine direkte
Verbindung) sind.
