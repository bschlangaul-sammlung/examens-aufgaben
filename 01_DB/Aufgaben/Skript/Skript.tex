...
\begin{enumerate}
\item Gegeben sind die Domänen Klassenstufe = \{5, 11\} und
Raumnummer = \{101, 102, 202\}\}. Bestimmen Sie das kartesische Produkt
Klassenstufe x Raumnummer.

\begin{antwort}[muster]
\{(5, 101), (5, 102), (5, 202), (11, 101), (11, 102), (11,202)\}
\end{antwort}
...
Lösung: \{(2017, 1, Max Mustermann, rk), (2018, 2, Sabine Kracker, ev)\}
...
\begin{antwort}[muster]
Klasse \{[Name: STRING, Klassenzimmer: STRING]\}
\end{antwort}
...
\begin{antwort}
Man könnte eine neue Spalte einfügen, z. B. bei Lehrer pensioniert.
\end{antwort}
...
$Artikel\{Lieferant, Adresse, Artikelname, Preis\}$ , die folgende
...
\}$
und die Menge
$X = \{Lieferant, Artikelname\}$.
Bestimmen Sie $AttrHull(F, X)$.

\begin{antwort}
$F^+ = \{Lieferant, Artikelname\}$

1. Durchlauf

$Lieferant$ ja $F^+ = \{Lieferant, Artikelname, Adresse\}$
$Lieferant Artikelname$ ja $F^+$ $F^+ = \{Lieferant, Artikelname, Adresse, Preis\}$

2. Durchlauf

keine Veränderungen mehr

$F^+ = \{Lieferant, Artikelname, Adresse, Preis\}$
\end{antwort}
...
\item Gegeben sei das Relationenschema $Abstrakt\{A, B, C, D, E\}$ , und
$F := \{
  A D \rightarrow C,
  B \rightarrow D,
  E \rightarrow A
\}$. Bestimmen Sie $AttrHull(F, \{A, B\})$.
\end{enumerate}
...
\item Gegeben sei das Relationenschema
\texttt{Professor\{PersNr, Dienststelle, Zimmer, Telefon\}}
und
\texttt{F = \{
...
\begin{itemize}
\item \texttt{Zimmer $\rightarrow$ Telefon}: \texttt{\{Zimmer\}} ist
...
\begin{itemize}
\item Wir berechnen:
\texttt{AttrHull(F, \{PersNr\}) = \{PersNr\}}, aber
\texttt{Zimmer $\notin$ \{PersNr\}}.

Dienststelle ist nicht redundant.

\item Wir berechnen:
\texttt{AttrHull(F, \{Dienststelle\}) = \{Dienststelle\}},
aber
\texttt{Zimmer $\notin$ \{Dienststelle\}}.
...
Wir berechnen:
\texttt{AttrHull(F - \{Zimmer $\rightarrow$ Telefon\}, \{Zimmer\}) = \{Zimmer\}},
aber
\texttt{Telefon $\notin$ \{Zimmer\}}.
...
\item \texttt{PersNr Dienststelle $\rightarrow$ Zimmer}:
Wir berechnen:
\texttt{AttrHull(F - \{PersNr Dienststelle $\rightarrow$ Zimmer\}, \{PersNr, Dienststelle\}) =
\{PersNr, Dienststelle\}},
aber
\texttt{Zimmer $\notin$ \{PersNr, Dienststelle\}}.
...
\texttt{Geburtsjahr} $\in$ \texttt{AttrHüll\{F, \{PersNr\}\} = \{PersNr, Name, Wohnort, Geburtsjahr\}}

\item \texttt{PersNr Name $\rightarrow$ Geburtsjahr} ohne \texttt{PersNr}:

\texttt{Geburtsjahr} $\in$ \texttt{AttrHüll\{F, \{Name\}\} = \{Name, PersNr, Wohnort, Geburtsjahr\}}
...
Gegeben sei das Relationenschema \texttt{Professor\{PersNr, Dienststelle, Zimmer, Telefon\}}.
...
\texttt{\{PersNr, Dienststelle\}} &
\texttt{Erg} wird initialisiert\\

\texttt{\{PersNr, Dienststelle\} $\cup$ \{Zimmer\}} &
wegen FD \texttt{PersNr Dienststelle $\rightarrow$ Zimmer}\\

\texttt{\{PersNr, Dienststelle, Zimmer\} $\cup$ \{Telefon\}} &
...
\begin{enumerate}
\item Gegeben sei das Relationenschema
$Artikel\{\textrm{Lieferant, Adresse, Artikelname, Preis}\}$
und die Menge
$F = \{\textrm{Lieferant} \rightarrow \textrm{Adresse}, \textrm{Lieferant Artikelname} \rightarrow \textrm{Preis} \}$.
Zeigen Sie, dass Artikel nicht 2NF ist!

\item Gegeben ist das Relationenschema
$Lehrkraft\{\textrm{PersNr, Name, Geburtsdatum, Wohnort}\}$
mit
$F = \{PersNr \rightarrow Name, PersNr \rightarrow Geburtsdatum, PersNr \rightarrow Wohnort, Name Geburtsdatum \rightarrow PersNr\}$.
